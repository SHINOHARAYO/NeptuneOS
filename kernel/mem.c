...
void mem_init(uint64_t sys_info)
{
    region_count = 0;
    managed_pages = 0;
    reserved_pages = 0;
    ...
    arch_mem_init(sys_info);
    
    if (region_count == 0) {
        panic("No physical memory regions found", 0);
    }
    
    for(uint32_t i=0; i<region_count; ++i) {
        if(regions[i].phys_end > max_phys_end) {
            max_phys_end = regions[i].phys_end;
        }
    }
